...
class UrbanAIServer {
    ...
    setupRoutes() {
        ...
        this.app.post('/api/process-documents', async (req, res) => {
            try {
                ...
                // Process documents
                const chunks = await this.documentProcessor.processAllDocuments(docsPath);
                
                // Store in vector database
                await this.vectorStore.upsertDocuments(chunks);
                
                // Save processed chunks
                await this.vectorStore.saveProcessedChunks(chunks, path.join(__dirname, '../docs'));
        ...
        this.app.post('/api/query', async (req, res) => {
            try {
                ...
                const result = await this.queryEngine.query(question, options);
        ...
        this.app.get('/api/summarize/:documentName', async (req, res) => {
            try {
                ...
                const documentPath = path.join(__dirname, '../docs', documentName);

                const summary = await this.queryEngine.summarizeDocument(documentPath);
        ...
        this.app.get('/api/summarize', async (req, res) => {
            try {
                ...
                const summary = await this.queryEngine.summarizeDocument(null);
        ...
        this.app.post('/api/extract-key-points', async (req, res) => {
            try {
                ...
                const result = await this.queryEngine.extractKeyPoints(topic);
        ...
        // Catch-all route for SPA (commented out for now)
    ...
    start() {
        this.app.listen(this.port, () => {
            console.log(`=� UrbanAI server running on port ${this.port}`);
            console.log(`=� Ready to process urban planning documents`);
            console.log(`= Query endpoint: http://localhost:${this.port}/api/query`);
            console.log(`=� Initialize system: POST http://localhost:${this.port}/api/initialize`);
        });
    }
}
