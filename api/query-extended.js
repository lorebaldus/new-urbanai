import { Pinecone } from '@pinecone-database/pinecone';                             
        ...
        try {
            ...
            if (command) {
                ...
                switch (command) {
                    ...
                    default:
                        return res.status(400).json({ success: false, error: 'Unknown 
    command' });
                }
            }
            ...
            if (searchResponse.matches && searchResponse.matches.length > 0) {
                console.log(`Found ${searchResponse.matches.length} relevant 
    documents`);
            ...
            let systemPrompt = `Sei UrbanAI, un assistente esperto in urbanistica, 
    edilizia e normative italiane. 
    Rispondi in modo professionale, pratico e preciso. Usa terminologia tecnica quando 
    appropriato ma spiega i concetti complessi.
    Concentrati su aspetti pratici come permessi, distanze, vincoli, zonizzazione e 
    ...
    Se non hai informazioni specifiche, fornisci comunque orientamenti generali utili.`;
            ...
            if (context && context.length > 100) {
                systemPrompt += `\n\nUtilizza le seguenti informazioni dalla Gazzetta 
    Ufficiale quando rilevanti:\n\n${context}`;
                userPrompt += '\n\nBasa la risposta sulle normative ufficiali quando 
    possibile.';
            }
            ...
            return res.status(500).json({
                ...
                message: 'Problema temporaneo con il servizio AI. Riprova tra qualche 
    istante.',
                fallback: true
            });
        }
    ...
    embedded: true });
            ...
            const completionRate = totalDocuments > 0 ? Math.round((processedDocuments /
     totalDocuments) * 100) : 0;
            ...
            return res.status(200).json({
                ...
                message: `System: ${totalDocuments} docs, ${processedDocuments} 
    processed (${completionRate}%)`
            });
    ...
    `gazzetta_${source}`);
            ...
            return res.status(200).json({
                ...
                message: `Processed ${processedResults.length} documents. 
    ${pdfUrls.length - maxDocs} remaining.`
            });
